...
export default function ExerciseCard({ exercise, index }) {
  ...
  return (
    <div className="p-2 m-2 rounded-md flex flex-col gap-4 bg-slate-950 sm:flex-row sm:items-center sm:flex-wrap gap-x-4 text-slate-100">
      <h4 className="text-3xl hidden sm:inline sm:text-4xl md:text-5xl font-semibold text-slate-300 shadow shadow-slate-800 p-2 rounded-md">
        0{index + 1}
      </h4>
      ...
      <p className=" text-slate-100 capitalize text-sm gap-1">{exercise?.type || ""}</p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-1 text-slate-200">
        <h3>Muscle:</h3>
        <p className="capitalize text-slate-100">{(exercise?.muscle || exercise?.muscles || []).join(" U ")}</p>
      </div>

      <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 p-2 ">
        <h3>Description</h3>
        <div className="capitalize flex flex-col gap-4 text-slate-100">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:place-items-center">
        {["reps", "rest", "tempo"].map((info) => (
          <div
            ...
            key={info}
          >
            <h3 className="capitalize text-slate-100 text-sm">
              {info === "reps" ? exercise?.unit || info : info}
            </h3>
            <p className="font-medium text-slate-400">{exercise?.[info] ?? "-"}</p>
          </div>
        ))}
